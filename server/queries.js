...
//const sqlGetOperatingSiteByTimestamp = "SELECT jsonb_build_object( 'type',     'FeatureCollection', 'features', jsonb_agg(feature) ) FROM ( SELECT jsonb_build_object( 'type',       'Feature', 'id',         id, 'geometry',   ST_AsGeoJSON(geom,5)::jsonb, 'properties', id ) AS feature FROM ( SELECT name as id, ST_Collect(ST_GeomFromWKB(location)) As geom FROM operating_site_history as sv WHERE  experiment_id = (select max(id) from experiment limit 1) GROUP BY name, location ) row) features;"

const sqlGetOperatingSiteByTimestamp = "SELECT jsonb_build_object( 'type',     'FeatureCollection', 'features', jsonb_agg(feature) ) FROM ( SELECT jsonb_build_object( 'type',       'Feature', 'id',         id, 'geometry',   ST_AsGeoJSON(geom,5)::jsonb, 'properties', jsonb_build_object('title',id, 'description',     id, 'marker-size', 'large','marker-color', '#ffffff'    )) AS feature FROM ( SELECT name as id, ST_AsGeoJSON(location) as geom FROM operating_site_history  WHERE  experiment_id = (select max(id) from experiment limit 1) GROUP BY name, location ) row) features;"
...
const sqlPathsOrderedByTimestamp = "SELECT jsonb_build_object( 'type',     'FeatureCollection', 'features', jsonb_agg(feature) ) FROM ( SELECT jsonb_build_object( 'type',       'Feature', 'id',         id, 'geometry',   ST_AsGeoJSON(geom,5)::jsonb, 'properties', id ) AS feature FROM ( SELECT id, st_makeline(ST_GeomFromWKB(current_location) ORDER BY timestamp asc) As geom FROM spontaneous_volunteers as sv WHERE experiment_id = (select max(id) from experiment limit 1) GROUP BY id  ) row) features;"
//const sqlPathsOrderedByTimestamp = "SELECT jsonb_build_object( 'type',     'FeatureCollection', 'features', jsonb_agg(feature) ) FROM ( SELECT jsonb_build_object( 'type',       'Feature', 'id',         id, 'geometry',   ST_AsGeoJSON(geom,5)::jsonb, 'properties', id ) AS feature FROM ( SELECT id, st_makeline(ST_GeomFromWKB(current_location)) As geom FROM spontaneous_volunteers as sv WHERE experiment_id = (select max(id) from experiment limit 1) GROUP BY id ) row) features;"
